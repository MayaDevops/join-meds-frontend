import { all, takeLatest, put, fork, take } from "redux-saga/effects";
  ...
  if (type === ACTION_TYPES.CREATE_JOB_DETAILS_SUCCESS) {
    ...
    yield put(
      commonActions.setAlertAction({
        ...
        forwardAction: () => {
          routeRedirect(`ui/join-meds/user/dashboard`);
        },
      }),
    );
    if (!_.isEmpty(responsePayLoad)) {
...
export function* fetchUserListDetails({ payload = {} }) {
  yield fork(handleAPIRequest, api.fetchUserListApi, payload);
  const { payload: { data: responsePayLoad = {} } = {}, type } = yield take([
    ACTION_TYPES.FETCH_USER_LIST_SUCCESS,
    ACTION_TYPES.FETCH_USER_LIST_FAILURE,
  ]);
  if (type === ACTION_TYPES.FETCH_USER_LIST_SUCCESS) {
    yield put(sliceActions.setUserListReports(responsePayLoad));
  }
}
